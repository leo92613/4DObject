...
public class MixedIKManager : MonoBehaviour {
	...
	Vector3 SolveForHipsPosition()
	{
		...
		else
		{
			...
			//Vector3 offsety = Vector3.up * Vector3.Magnitude(offsetxz) * (0.2f + Mathf.Abs (Vector3.Dot (offsetxz.normalized, _hipJoint.transform.forward.normalized)) * 1.0f);
			hit += offsetxz;// + offsety;
			//hit = Vector3.Lerp(_nb,_headJoint.transform.position,.5f);
			//hit = Vector3.Scale(hit,new Vector3(1,1,-.5f));
