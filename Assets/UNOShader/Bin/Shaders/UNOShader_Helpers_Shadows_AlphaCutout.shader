	...
	SubShader 
	{
		Pass 
		{
			...
			{	
				...
				fixed4 T_Emission = tex2D( _EmissionMap, i.uv3 );
				fixed result = T_Diffuse.a;
				fixed colorAlpha = lerp(1,0, _Color.a);
				result = lerp(0,1,(T_Diffuse.a * _MainTexOpacity + _Color.a));
				result = lerp( result,1,T_Decal.a * _DecalColor.a );
				result = lerp( result,1,T_Emission.a * _EmissionColor.a );
				clip( result - _Cutoff );
		...
		Pass 
		{
			...
			fixed4 frag (v2f i) : COLOR
			{
				...
				fixed result = T_Diffuse.a;
				result = lerp(0,_Color,(T_Diffuse.a * _MainTexOpacity));
				result = lerp( result,1,T_Decal.a * _DecalColor.a );
